...
// 浏览器持久化键：vite/普通静态服务器没有 omelette runtime 时的主存储。
// 避免对不存在的 sidecar 文件硬打 fetch（会在 DevTools 控制台留 404 噪音）。
const DC_LOCALSTORAGE_KEY = 'rpg.design-canvas.state.v1';
...
function DesignCanvas({ children, minScale, maxScale, style }) {
  ...
  React.useEffect(() => {
    let off = false;
    // 1) localStorage 是主存储：首次/任意环境都不会产生网络请求
    let hydrated = null;
    try {
      const raw = localStorage.getItem(DC_LOCALSTORAGE_KEY);
      if (raw) {
        const parsed = JSON.parse(raw);
        if (parsed && parsed.sections) hydrated = parsed;
      }
    } catch (_) { /* localStorage 不可用 / JSON 损坏 → 当无状态 */ }
    if (hydrated) {
      skipNextWrite.current = true;
      setState((s) => ({ ...s, sections: hydrated.sections }));
    }

    // 2) 只在 omelette runtime 里才去拉 sidecar 文件（其他环境压根不存在，
    //    硬打 fetch 会让浏览器记一条 404，污染审计控制台）。omelette runtime
    //    暴露 window.omelette 作为 writeFile 桥；探测同一对象即可。
    if (window.omelette) {
      fetch('./' + DC_STATE_FILE)
        .then((r) => (r.ok ? r.json() : null))
        .then((saved) => {
          if (off || !saved || !saved.sections) return;
          // sidecar 优先于 localStorage（omelette 是共享视图的权威源）
          skipNextWrite.current = true;
  ...
  React.useEffect(() => {
    ...
    const t = setTimeout(() => {
      // localStorage：所有环境都写，让用户在普通浏览器里也能保持画板状态
      try {
        localStorage.setItem(DC_LOCALSTORAGE_KEY, JSON.stringify({ sections: state.sections }));
      } catch (_) { /* quota / disabled → 静默 */ }
      // sidecar：仅 omelette 环境才写（writeFile 桥不在就跳过）
