...
/* ============================================================
   ...
   架构变化:
   - 身份卡 = save 级独立 overlay,*不修改* 所选角色卡的 name/role/外貌等。
   - 默认 identity=null = 直接使用所选角色卡,不挂身份 overlay。step 必通过。
   - AI 生成是 **opt-in**:用户主动点按钮才请求,默认不自动拉。
   - LLM 失败/上下文不足时,backend 返 5xx + 真实错误,前端如实显示,
     引导用户改走手动创建或直接用角色卡。不再有硬编码的「剧本主角/局外观察者」兜底。
   - 手动创建身份卡:name 可选(代号/化名),role + background 是身份本体。
   ============================================================ */
...
function NewGameModal({ open, onClose, onConfirm, defaultScriptId = null }) {
  ...
  // v27: 身份卡是可选 overlay,留空 = 直接使用所选角色卡。step 不再硬卡 identity 必填。
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const payload = {
        ...
        // v27: 透传 source (custom|ai) 给后端落库,identity 留空 = 不挂身份 overlay
        identity: identity ? {
          name: identity.name || "",
          role: identity.role || "",
  ...
  /* ── step labels ── */
  const stepLabels = ["剧本", "角色", "出生点", "初始身份 (可选)", "剧情期望"];
