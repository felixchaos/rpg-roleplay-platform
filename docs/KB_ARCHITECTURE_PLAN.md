...
## 0.5 与现有代码的对账(v5 关键:别重造已有的轮子)
> 探查 `rpg/` 后确认:**世界树骨架、收束锚点、工具框架、世界书都已实现且能用**。提取/检索/引导是真缺口。

| 能力 | 现状 | 结论 |
|---|---|---|
| 提交 DAG(世界树脊柱) | `branch_commits(id, parent_id, state_snapshot jsonb, turn_index, kind...)` 已是 git 式 DAG;`branch_refs` 头指针;`runtime_checkouts` 每用户工作树 dirty 标记 | **复用脊柱**。但现在每 commit 存**整份 state JSON 快照**(full-snapshot COW),不是行级版本化。关系型 KB 层(§2)是**新增层**,挂在同一 `commit_id` 上 |
| 收束/锚点 | `save_anchor_states`(importance/is_fatal/status/drift_score)+ `mark_anchor_satisfied/superseded` 工具 + `anchor_seed_agent` 从 chapter_facts.events 逐事件播种**已存在** | **复用**。规范世界线 DAG(§4.4)是**更粗**的弧级结构,坐在细粒度 anchor 之上;GM 粗粒度引导、细粒度保真 |
| 世界书 | `worldbook_entries`(keys/regex_keys/priority/probability/insertion_position/character_filter)+ `save_worldbook_overlays`(addition/retirement) | **复用**。新增 `insertion_position='constant'` 模式做常驻骨架(§4.1) |
| 时间线 | `script_timeline_anchors`(phase→章节范围,linear)+ `save_timeline_anchors`(每回合标注) | **复用**,但 linear。纪元当**钉死种子常量**(治 1935)走 §4.1 constant |
...
**一句话**:脊柱(DAG)、收束(anchor)、工具(dispatcher)、世界书已就位。本架构补:① 干净的料(§3)② 真实提取(§6)③ 关系型 KB 行级层(§2)④ 规范世界线粗 DAG(§4.4)⑤ 真向量检索(§6 Pass3)⑥ 有界 GM loop + 常驻骨架 + 防剧透/元知识(§7)⑦ 可视化编辑器(§8)。
...
## 2. 数据模型:关系型世界树(在现有 commit DAG 上加行级 KB 层)
现状:`branch_commits` 已是 commit DAG,但每 commit 存整份 state JSON。**新增**行级版本化 KB 层,挂同一 `commit_id`:
```sql
-- 复用(已存在):branch_commits(id, save_id, parent_id, turn_index, state_snapshot, ...)
-- 新增:活态 KB 行级表,每行打 born_commit 戳(= 写入时的 active commit_id)
kb_entities(id, save_id, born_commit, logical_key, name, type, status, summary, attrs jsonb, retired_at_commit, ...)
kb_events(id, save_id, born_commit, story_time, summary, participants jsonb, location, retired_at_commit, ...)
kb_relationships(id, save_id, born_commit, from_key, to_key, kind, note, retired_at_commit, ...)
...
- **fork = 零拷贝**(只 fork commit 指针,KB 行不动);GM 改实体 = 只 INSERT 一行(打当前 commit 戳);分支天然隔离
- 存储 O(变化数),版本化零 LLM 成本。索引 `(save_id, logical_key, born_commit)`、`(save_id, born_commit)`
- 同 Dolt/Datomic/SQL:2011 system-versioned tables。删除用 `retired_at_commit` tombstone
- **与 full-snapshot 并存**:`state_snapshot` 继续存玩家可见的轻量运行态(player/scene/turn);重型世界知识进 `kb_*` 行级层,避免每回合复制整个世界。详见 `docs/design/BC_kb_schema_worldtree.md`
