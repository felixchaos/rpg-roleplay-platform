...
### 1.1 实测结果(二战书 11.4MB 原始 TXT,LIVE 验收 2026-05-30)
| 指标 | 目标 | 实测 | 结论 |
|---|---|---|---|
| 内容保全(非空白字符 vs sanitized) | 不丢 | **100.09%** | ✅ **零内容丢失**(真·漏切=0) |
| 广告清洗 | 强于旧实现 | ad 行 **2492** 删除,正文/描述符**零残留** | ✅(发现并补回 port 时漏的 啃书/KenShu 规则) |
| 非正文(author_note) | <2% | **4.0%**(34 章) | ⚠️ 见下:原 21.5% 把"玩梗标题真章"误算进去了 |
| 怪标题(低可信,生成描述符) | 标注非删 | **18.6%**(159 章,KenShu 描述符已修为真内容) | ✅ 真章保留+描述符 |
| 编号 gap(review flag) | 不静默 | 120 处 flag,`needs_review=True` | ✅ 全上报人工复核(此书双重编号 第3章003 + 分页（1）-（10）天然不连续) |
| 书本特化 | 0 | 删 REMULINA 后此书经**通用** strong_headings/derived 规则切分 | ✅ 换书不退化 |

**关于"非正文 4% 未达 <2%"的诚实说明**:原 21.5%(186 章)的估计**把"玩梗标题的真剧情章"也算成了非正文**。
A0 把二者**正确分开**:真·非正文(卷末通知等)= 4.0%(34 章,`exclude_from_extraction=true`,提取看不到);
玩梗标题的**真章** = 18.6%(159 章,**保留**,只是 `title_confidence` 低 + 生成内容描述符)。
所以"喂进提取的非正文" ≈ **0**(34 章全部 exclude),**达成 <2% 的本意**(治污染);4% 是这本书真实的卷末通知占比,不该靠误删压到 2%。
**铁律守住**:不针对此书调参(120 gap 是此书畸形编号的诚实反映,不为压指标而特化)。
