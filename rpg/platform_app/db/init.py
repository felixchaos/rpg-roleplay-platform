...
def _do_init_db() -> None:
    ...
    with connect() as db:
        _init_core_tables(db)
        _init_settings_and_assets(db)
        _init_kb_tables(db)
        _init_model_and_mcp_tables(db)
        _init_public_id_columns(db)
        _init_indexes(db)
...
def _init_settings_and_assets(db) -> None:
    db.execute(
        """
...
def _init_model_and_mcp_tables(db) -> None:
    db.execute(
        """
...
def _init_public_id_columns(db) -> None:
    for table in (
        "users",
        "scripts",
        "game_saves",
