...
MIGRATIONS: list[tuple[int, str, list[str]]] = [
    ...
    (91, "acceptance_ab_log", [
        # acceptance A/B 改写候选采集层:GM 首稿有 acceptance 未覆盖且节流放行时,生成一份「改写候选」
        # 与首稿并排给玩家选择(不再静默替换)。每次提供候选落一行(chosen=null 待选),玩家选择后
        # 回写 chosen —— 用来统计「玩家更偏好首稿还是改写稿 / 哪些验收点最常触发改写」以迭代 acceptance 算法。
        # 首稿=original_text(权威落库版),改写=rewrite_text(服务端存,玩家选它才换消息,前端不回传正文防注入)。
        "create table if not exists acceptance_ab_log ("
        "  id bigint generated by default as identity primary key,"
        "  user_id bigint references users(id) on delete cascade,"
        ...
        "  chosen text,"          # null=未选 / 'original' / 'rewrite'
        "  created_at timestamptz not null default now(),"
    ...
    (102, "provider_failures_telemetry", [
        # 「错误有没有被拦截器捕获」这个问题此前**从数据库里答不了**:provider 失败只进
        # 日志和 SSE,一条都不落库(近 60 天 token_usage 里 0 条错误记录)。
        # _client_safe_error 是所有流式错误的唯一漏斗,它已经分好类、生成了 error_id、
        # 写了日志 —— 只差落库。这张表让「拦截率 / 未分类占比 / 哪个渠道在坏」变成可查数字。
        #
        # 刻意**不复用 token_usage**:那是计费账本,塞进零 token 的失败行会污染用量 UI 与
        # 汇总口径。这里是纯遥测,与钱无关。
        """
