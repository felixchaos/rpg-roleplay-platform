...
def import_save(user_id: int, payload: dict[str, Any]) -> dict[str, Any]:
    ...
    with connect() as db:
        ...
        # 2. 重建 branch_commits / 3. 重建 branch_refs / 4. 导入 9 张 per-save 状态表
        old_to_new = _import_branch_commits(db, payload, new_save_id)
        _import_branch_refs(db, payload, new_save_id, old_to_new)
        state_imported = _import_state_tables(db, payload, new_save_id, old_to_new, pv, save_data, warnings)
...
def _import_branch_commits(db, payload, new_save_id) -> dict[int, int]:
...
def _import_branch_refs(db, payload, new_save_id, old_to_new) -> None:
...
def _import_state_tables(db, payload, new_save_id, old_to_new, pv, save_data, warnings) -> dict[str, int]:
