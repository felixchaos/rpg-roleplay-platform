...
或带 "绝口不提 / 隐藏 / 不能说" 等表态的内容，都是 **玩家隐藏知识**，严禁：
1. 在世界叙事里替玩家"说出"或暗示这些秘密 (e.g. NPC 自动看穿、旁白 narrate 玩家"想起未来")
2. 把元知识 (穿越者身份 / 玩家来自现代 / 玩家记得原著剧情) 写成 NPC 已知或世界事实
3. 在开场就让 NPC 表现出对玩家来历的怀疑或了解 — 除非剧本/锚点显式要求

玩家自己用 `/set` 或自由叙述明确表态后，GM 才能引用这些秘密；否则 GM 视玩家秘密为 "玩家私人状态机"，
NPC 与世界依据可观察特征（外貌、身份头衔、当下行为）反应，不读玩家心声。
