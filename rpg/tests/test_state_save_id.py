...
class StateExposesSaveContext(unittest.TestCase):
    ...
    def test_state_contains_save_id_after_save_active(self):
        ...
        r_new = self.client.post("/api/new", json={
            "name": "integtest_player",
            "role": "tester",
            "background": "测试 player",
        }, cookies=cookies)
        self.assertEqual(r_new.status_code, 200, f"/api/new failed: {r_new.text[:200]}")

        # /api/state 现在应该带 save_id + save_title
        r_state = self.client.get("/api/state", cookies=cookies)
        self.assertEqual(r_state.status_code, 200)
        body = r_state.json()
        self.assertIn("save_id", body, f"/api/state 必须暴露 save_id（task 10 契约锚）：{list(body)[:20]}")
    ...
    def test_state_save_id_matches_saves_list_first(self):
        u = register_user(self.client)
        cookies = u["cookies"]
        self.client.post("/api/new", json={"name": "p2", "role": "r", "background": ""}, cookies=cookies)
        r_state = self.client.get("/api/state", cookies=cookies)
