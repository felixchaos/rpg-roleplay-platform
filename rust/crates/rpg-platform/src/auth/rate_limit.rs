...
/// 对应 Python `_write_audit` — 写一行 `login_audit`。
///
/// 表 `login_audit` 由 migration 007 建立:
/// ```sql
...
/// 对应 Python:
/// ```python
/// db.execute("insert into login_audit(username, ip, event, meta) values (%s, %s, %s, %s)",
///            (username, ip, event, Jsonb(meta)))
/// ```
///
/// 任何 DB 异常都吞掉(与 Python `except Exception: pass` 一致),确保登录主路径不因审计失败而中断。
...
/// 带 DB audit 的 admin_unlock。路由层在有 pool 时调用此版本。
///
/// 对应 Python `admin_unlock` 里的 `_write_audit(username, ip, "admin_unlock", {})`.
pub async fn admin_unlock_with_audit(
    pool: &sqlx::PgPool,
    ip: &str,
    username: &str,
) {
    GLOBAL_LIMITER.admin_unlock(ip, username).await;
    record_login_audit(pool, username, ip, "admin_unlock", &serde_json::Value::Object(Default::default())).await;
}
...
mod tests {
    ...
    /// bucket_key 对 ip/username 的格式化不变(验证 fail_key/lock_key 稳定)。
    #[test]
    fn bucket_key_format() {
        // ip 空时用 "-"
        assert_eq!(bucket_key("", "Alice"), "-|alice");
        // username 应小写化
        assert_eq!(bucket_key("1.2.3.4", "BOB"), "1.2.3.4|bob");
    }

    /// fail_key / lock_key 有确定前缀。
    #[test]
    fn key_prefixes() {
        assert!(fail_key("x").starts_with("login:fail:"));
        assert!(lock_key("x").starts_with("login:lock:"));
    }
}
