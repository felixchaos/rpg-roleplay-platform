...
/// 迁移 017 — sessions 会话表(token_hash 存 SHA-256 hex,不存明文)。
static SQL_017: &str = concat!(
...
/// 迁移 018 — stop_signals 跨进程取消信号表(对应 cluster.rs::request_stop)。
static SQL_018: &str =
...
/// 迁移 019 — runtime_checkouts state 缓存时间戳表(对应 cluster.rs::is_state_stale)。
static SQL_019: &str = concat!(
